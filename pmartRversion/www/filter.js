...
(function() {
  ...
  filter = (function() {
      ...
      db.open({
        ...
        return function(s) {
          ...
          return s.biom.query().all().execute().done(function(results) {
            ...
            _this.generateDate();
            $("#file_details").append("<i class='icon-download' id='goExport'></i><span>" + filename.substring(0, 52) + "</span><br />OBSERVATIONS &nbsp;&nbsp;&nbsp;<span>" + format(biom.shape[0]) + "</span> <br /> SELECTED SAMPLES &nbsp;&nbsp;&nbsp;<span>" + format(biom.shape[1]) + "</span>");
    ...
    filter.prototype.livePreview = function() {
      ...
      content = "<table id='myTable'><thead><tr>"//"<th class = 'headerID myTableHeader'>PHINCH NAME</th>"
      +"<th class = 'headerID myTableHeader'>BIOM SAMPLE ID" + "</th><th class='myTableHeader'>SAMPLE NAME</th><th class='headerCount myTableHeader'>SEQUENCE READS</th></thead>";
      if (this.selected_sample.length > 0) {
        for (i = af = 0, ref15 = this.selected_sample.length - 1; 0 <= ref15 ? af <= ref15 : af >= ref15; i = 0 <= ref15 ? ++af : --af) {
          content += '<tr>' //'<td id="phinchID_' + this.selected_sample[i] + '">' + phinchID_array[this.selected_sample[i]] + '</td>'
          + '<td>' + (this.selected_sample[i] + 1) + '</td><td>' + columns_sample_name_array[this.selected_sample[i]] + '</td><td>' + columns_sample_count_list[this.selected_sample[i]] + '</td></tr>';
        }
      }
      content += "</table>";
