...
shinyServer(function(input, output, session) {
  ...
  output$max_indsp_grp <- renderUI({
    sliderInput("max_indsp_grp",
                label = "Max number of indicator species groups",
                min = 1,
  ...
  #observeEvent(input$submit_is, {
    indsp_res <<- reactive({
      req(input$submit_is)
      if(input$within == "NA"){
        return(pmartRseq::indsp_calc(omicsData = normalized_data(), within = NULL, pval_thresh = input$is_pval_thresh, max_grp = input$max_indsp_grp))
      }else{
        return(pmartRseq::indsp_calc(omicsData = normalized_data(), within = input$within, pval_thresh = input$is_pval_thresh, max_grp = input$max_indsp_grp))
      }
    })
